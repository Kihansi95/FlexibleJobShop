...
public class GanttChart extends JScrollPane implements GanttModelListener, PropertyChangeListener, SelectionChangeListener {
	...
	private TimeUnit timeUnit = TimeUnit.defaultUnit;// Singleton
	...
	public GanttChart() {
		...
		//		DebugConsole.setDebug(true); // Set to true if wanna ouput debug infor in console.
		...
		//		label.setText("SWIFTGANTT");
		//		this.setCorner(JScrollPane.UPPER_LEFT_CORNER, label);

		// Listen selection change and to refresh UI
	...
	public void ganttModelChanged(GanttModelChangeEvent e) {
		...
		if (layoutSuspendCount == 0) {
			...
			if (logger.isInfoEnabled()) {
				logger.info("ganttModelChanged(GanttModelChangeEvent) - int layoutSuspendCount=" + layoutSuspendCount); //$NON-NLS-1$
			}
		}
	}
	...
	public void propertyChange(PropertyChangeEvent evt) {
		//		DebugConsole.event(evt, "The value of config " + evt.getPropertyName() + " has been changed from " + evt.getOldValue() + " to " + evt.getNewValue() + "!");
		//		this.ganttChartView.refreshDisplay();
		//		this.taskTreeView.refreshDisplay();
		//		this.timeScaleView.refreshDisplay();
	}
	...
	public void generateImageFile(String fileName) throws IOException {
		...
		Dimension mainDimension = new Dimension(gcView.getBounds().width + ttView.getBounds().width, tsView.getBounds().height
				+ gcView.getBounds().height);
		...
		Rectangle gcRect = new Rectangle(ttView.getBounds().width, tsView.getBounds().height, gcView.getBounds().width,
				gcView.getBounds().height);
		Rectangle logoRect = new Rectangle(0, 0, ttRect.width, tsRect.height);
		logoView.setBounds(logoRect);// Set it's bounds because it will not adjust it's bound.
		...
		if (ext.equals("png")) {
			SwingComImageGenerator.getInstance().genPNGImage(mainDimension, new JComponent[] { gcView, tsView, ttView, logoView },
					new Rectangle[] { gcRect, tsRect, ttRect, logoRect }, fileName);
		} else if (ext.equals("jpg")) {
			SwingComImageGenerator.getInstance().genJPEGImage(mainDimension, new JComponent[] { gcView, tsView, ttView, logoView },
					new Rectangle[] { gcRect, tsRect, ttRect, logoRect }, fileName);
		} else {
	...
	public void setModel(GanttModel ganttModel) {
		...
		this.taskTreeView.refreshDisplay();
		this.ganttChartView.refreshDisplay();
		//		logger.debug("New model attached to GanttChart");
	}
	...
	protected void calcTotalSteps(Time kickoffTime, Time deadline) {
		//( I think it is better to move to the GanttModel, because model should know how many steps)
		int ret = 0;
		...
		totalSteps = totalScheduleSteps
				+ (config == null ? Config.DEFAULT_BLANK_STEPS_TO_KICKOFF_TIME : config.getBlankStepsToKickoffTime());
		totalSteps += config == null ? Config.DEFAULT_BLANK_STEPS_TO_DEADLINE : config.getBlankStepsToDeadline();

		//Enlarge the steps if the width of Gantt chart is less than the visible area.
	...
	public void suspendUI() {
		if (logger.isInfoEnabled()) {
			logger.info("suspgendUI() - Suspend UI - layoutSuspendCount=" + layoutSuspendCount); //$NON-NLS-1$
		}
		layoutSuspendCount++;
	}
	...
	public void resumeUI() {
		if (logger.isInfoEnabled()) {
			logger.info("resumeUI() - Resume UI - layoutSuspendCount=" + layoutSuspendCount); //$NON-NLS-1$
		}
		if (layoutSuspendCount > 0) {
			if (--layoutSuspendCount == 0) {
				performLayout();
